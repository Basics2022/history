...
def update_color_by(c_key, political_data):

    political_data['color'], political_color_scale = 0., []

    c_by_k, c_by_i = c_key, color_by[c_key]

    n_index = 0
    n_try, n_except = 0, 0
    for index, row in political_data.iterrows():
        try:
        ...
        political_data.at[index, 'color'] = float(index)/float(max_index)
        political_color_scale += [ [index/max_index, rgb] ]
        n_index += 1

    return political_color_scale, political_data
...
def plot_map_data(political_data, history_c, history_p, pol_color_scale):
    political_trace = go.Choroplethmapbox(
        geojson  =political_data.geometry.__geo_interface__,
        locations=political_data.index,
        z=political_data['color'],
        # colorscale=color_scale,
        colorscale=pol_color_scale,
        zmin=0, zmax=1,
        text=political_data[hover_data].astype(str).agg(lambda x: '<br>'.join(x), axis=1),  # Use HTML line breaks
        #hoverinfo='text',
        marker=dict(opacity=0.5),
    )
